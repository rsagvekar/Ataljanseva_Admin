...
import {
  ...
  RefreshControl, useWindowDimensions, Alert, TextInput,
...
import {
  ...
  Card, FilterChips, PrimaryButton, SectionHeader,
} from '../../components/common/UI';
import { FormGroup, Input, SelectField, ModalSheet } from '../../components/layout/ScreenWrapper';
import { COLORS, TYPOGRAPHY, SPACING, RADIUS, SHADOWS } from '../../config/theme';
import { formatDate, truncate } from '../../utils/formatters';

const CATEGORIES = ['Water', 'Road', 'Drainage', 'Garbage', 'Streetlight', 'Health', 'Electricity', 'Building', 'Other'];
const STATUSES = ['Pending', 'In Progress', 'Resolved'];
const CAT_EMOJI = { Water: '💧', Road: '🛣️', Drainage: '🌊', Garbage: '🗑️', Streetlight: '💡', Health: '🏥', Electricity: '⚡', Building: '🏢', Other: '📌' };

const defaultForm = {
  name: '', mobile: '', category: 'Water', description: '',
  address: '', status: 'Pending', assignedTo: '', resolutionNote: '', photoUrl: '', ward: '',
};
...
export default function GrievancesScreen({ navigation }) {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const [gRes, vRes] = await Promise.all([
        ...
        volunteersService.getAll({ status: 'Active', limit: 200, nagarsevak_id: nagarsevakId }),
      ]);
  ...
  const openAdd = () => { setEditItem(null); setForm(defaultForm); setShowForm(true); };
  const openEdit = (g) => { setEditItem(g); setForm({ ...defaultForm, ...g }); setShowForm(true); };

  const filtered = items.filter((g) => {
    const s = search.toLowerCase();
    return (
      (!s || g.name?.toLowerCase().includes(s) || g.ticketId?.toLowerCase().includes(s) || g.mobile?.includes(s))
  ...
  const renderItem = ({ item: g }) => (
    ...
    >
      ...
      <View style={styles.itemMeta}>
        <Text style={styles.catBadge}>{CAT_EMOJI[g.category] || '📌'} {g.category}</Text>
        ...
        <View style={styles.actions}>
          {canUpdateStatus && (
            <TouchableOpacity
              ...
              onPress={() => {
                const next = g.status === 'Pending' ? 'In Progress' : g.status === 'In Progress' ? 'Resolved' : 'Pending';
    ...
    <View style={[styles.container, { paddingHorizontal: padding }]}>
      ...
      {toast && (
        <View style={[styles.toast, toast.type === 'error' ? styles.toastError : styles.toastSuccess]}>
          <Icon name={toast.type === 'error' ? 'alert-circle' : 'checkmark-circle'} size={16} color="#fff" />
      ...
      <View style={[styles.statsGrid, { paddingTop: padding }]}>
        {[
          { icon: '📋', label: 'Total', value: stats.total, color: '#dbeafe' },
          { icon: '⏳', label: 'Pending', value: stats.pending, color: '#fef3c7' },
          { icon: '🔄', label: 'In Progress', value: stats.inProgress, color: '#e0f2fe' },
      ...
      <Text style={styles.count}>{filtered.length} of {items.length} grievances</Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState icon="📋" title="No grievances found" subtitle="Try adjusting your filters." />}
        />
      )}

      {/* Detail Modal */}
      <ModalSheet visible={!!viewItem} onClose={() => setViewItem(null)} title="Grievance Details">
        {viewItem && (
          <View>
            ...
            {[
              ...
              ['Category', `${CAT_EMOJI[viewItem.category] || ''} ${viewItem.category}`],
              ['Ward', viewItem.ward],
            ...
            {viewItem.resolutionNote ? (
              <>
                ...
                <Text style={styles.detailDesc}>{viewItem.resolutionNote}</Text>
              </>
            ) : null}
            {viewItem.photoUrl ? (
              <Image source={{ uri: viewItem.photoUrl }} style={styles.detailPhoto} resizeMode="cover" />
            ) : null}
            {canUpdateStatus && (
              <View style={{ marginTop: SPACING.xl, gap: SPACING.md }}>
                {STATUSES.filter((s) => s !== viewItem.status).map((s) => (
                  <PrimaryButton
                    ...
                    onPress={() => { handleQuickStatus(viewItem, s); setViewItem(null); }}
                  />
                ))}
              </View>
            )}
      ...
      >
        ...
        <FormGroup label="Address">
          <Input value={form.address} onChangeText={(v) => setForm((f) => ({ ...f, address: v }))} placeholder="Location/address" multiline numberOfLines={2} />
        </FormGroup>
        ...
        {canAssign && volunteers.length > 0 && (
          <FormGroup label="Assign To">
            <SelectField
              ...
              options={[{ label: '— None —', value: '' }, ...volunteers.map((v) => ({ label: v.name, value: v.name }))]}
        ...
        <PrimaryButton title={editItem ? 'Update Grievance' : 'Add Grievance'} onPress={handleSave} loading={saving} style={{ marginTop: SPACING.md }} />
      </ModalSheet>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  ticketId: { ...TYPOGRAPHY.mono, color: COLORS.gray600, backgroundColor: COLORS.gray100, paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 },
  itemName: { ...TYPOGRAPHY.label, color: COLORS.gray800 },
  itemMobile: { ...TYPOGRAPHY.caption, color: COLORS.gray500 },
  itemMeta: { flexDirection: 'row', gap: SPACING.sm, marginBottom: SPACING.sm, flexWrap: 'wrap' },
  catBadge: { ...TYPOGRAPHY.caption, color: COLORS.gray600, backgroundColor: COLORS.gray100, paddingHorizontal: 8, paddingVertical: 3, borderRadius: RADIUS.full },
  ward: { ...TYPOGRAPHY.caption, color: COLORS.info, backgroundColor: COLORS.infoLight, paddingHorizontal: 8, paddingVertical: 3, borderRadius: RADIUS.full },
  desc: { ...TYPOGRAPHY.bodySm, color: COLORS.gray500, marginBottom: SPACING.sm, lineHeight: 18 },
  ...
  // Detail Modal
  detailHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: SPACING.lg },
  detailTicket: { ...TYPOGRAPHY.mono, fontSize: 14, color: COLORS.gray700, backgroundColor: COLORS.gray100, padding: 6, borderRadius: 6 },
  ...
  detailVal: { ...TYPOGRAPHY.bodySm, color: COLORS.gray800, fontWeight: '500', flex: 1, textAlign: 'right' },
  ...
  detailPhoto: { width: '100%', height: 200, borderRadius: RADIUS.lg, marginTop: SPACING.lg },
