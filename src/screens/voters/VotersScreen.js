...
export default function VotersScreen() {
  ...
  const load = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        const res = await votersService.getAll({
  ...
  const handleDelete = v => {
    Alert.alert('Delete Voter', `Delete ${v.name}?`, [
      {text: 'Cancel', style: 'cancel'},
      {
        ...
        onPress: async () => {
          try {
            await votersService.delete(v.id);
        ...
        <View style={styles.cardHeader}>
          ...
          <View style={styles.cardInfo}>
            ...
            <Text style={styles.cardSub}>
              {v.mobile} {v.ward ? `· Ward ${v.ward}` : ''}
            </Text>
            {v.voterId ? (
              <Text style={styles.cardVoterId}>ID: {v.voterId}</Text>
            ) : null}
          </View>
        ...
        <View style={[styles.container, {paddingHorizontal: padding}]}>
          ...
          <View style={styles.filterRow}>
            <SearchBar
              ...
              placeholder="Name, mobile, voter ID..."
              style={{flex: 1}}
            />
            {canManage && (
              <TouchableOpacity
          ...
          {loading ? (
            <LoadingSpinner text="Loading voters..." full />
          ) : (
            <FlatList
              data={filtered}
              keyExtractor={v => v.id}
              ...
              ListEmptyComponent={
                <EmptyState icon="🗳️" title="No voters found" />
              }
            />
          )}
          ...
          <ModalSheet
            ...
            {viewItem && (
              <View>
                <View style={{alignItems: 'center', marginBottom: SPACING.xl}}>
                  ...
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          (TYPE_COLORS[viewItem.voterType] || {}).bg ||
                          '#f3f4f6',
                    ...
                    <Text
                      style={[
                        ...
                        {
                          color:
                            (TYPE_COLORS[viewItem.voterType] || {}).text ||
                            '#4b5563',
                        },
                      ]}>
                      {viewItem.voterType}
                ...
                {viewItem.notes ? (
                  <>
                    <Text style={styles.detailDescLabel}>Notes</Text>
                    <Text style={styles.detailDesc}>{viewItem.notes}</Text>
                  </>
                ) : null}
              </View>
            )}
          ...
          <ModalSheet
            ...
            title={editItem ? 'Edit Voter' : 'Add Voter'}>
            ...
            <FormGroup label="Voter ID">
              <Input
                value={form.voterId}
                onChangeText={v => setForm(f => ({...f, voterId: v}))}
            ...
            <FormGroup label="Voter Type">
              <SelectField
                ...
                options={VOTER_TYPES}
                onChange={v => setForm(f => ({...f, voterType: v}))}
              />
            </FormGroup>
            ...
            <TouchableOpacity
              ...
              <Text style={styles.saveBtnText}>
                {saving ? 'Saving...' : editItem ? 'Update Voter' : 'Add Voter'}
              </Text>
            </TouchableOpacity>
          </ModalSheet>
        </View>
