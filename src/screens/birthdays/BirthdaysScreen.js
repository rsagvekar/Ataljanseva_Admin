...
export default function BirthdaysScreen() {
  ...
  const defaultBdayMsg =
    'जन्मदिनाच्या हार्दिक शुभेच्छा {name}जी! 🎂 तुमचे दीर्घायुष्य आणि उत्तम आरोग्यासाठी प्रार्थना. - तुमचा नगरसेवक';
  const defaultAnnivMsg =
    'वैवाहिक वर्धापनदिनाच्या हार्दिक शुभेच्छा {name}जी! 💍 सुखी दाम्पत्य जीवनासाठी शुभकामना. - तुमचा नगरसेवक';
    ...
    <View key={voter.id} style={styles.voterCard}>
      ...
      <View style={styles.voterInfo}>
        ...
        <Text style={styles.voterMeta}>
          {voter.mobile} {voter.ward ? `· Ward ${voter.ward}` : ''}
        </Text>
        <Text style={styles.voterDob}>
          {isBday
            ? `🎂 DOB: ${formatDate(voter.dob)}`
            : `💍 Anniv: ${formatDate(voter.anniversary)}`}
        </Text>
      </View>
      {voter.mobile && (
        <TouchableOpacity
          style={styles.waBtn}
          onPress={() => sendWhatsApp(voter, isBday)}>
      ...
      <View style={[styles.container, {paddingHorizontal: padding}]}>
        <ScrollView
          ...
          <StatCard
            icon="💍"
            label="Anniversaries"
            value={anniversaryVoters.length}
            color="#dbeafe"
            style={{flex: 1}}
          />
          <StatCard
        ...
        {loading ? (
          ...
          <FlatList
            ...
            scrollEnabled={false}
            renderItem={({item}) =>
              renderVoterCard(item, tab !== 'anniversaries')
            }
