...
import { StatCard, StatusBadge, SearchBar, EmptyState, LoadingSpinner, FilterChips } from '../../components/common/UI';
import { ModalSheet, FormGroup, Input, SelectField } from '../../components/layout/ScreenWrapper';
import { COLORS, TYPOGRAPHY, SPACING, RADIUS, SHADOWS } from '../../config/theme';
import { formatDate, formatDateTime } from '../../utils/formatters';

const EVENT_TYPES = ['Meeting', 'Rally', 'Cultural', 'Sports', 'Social', 'Health Camp', 'Other'];
const defForm = { title: '', type: 'Meeting', date: '', time: '', venue: '', description: '', expectedAttendance: '', organizer: '', status: 'Upcoming' };
...
export default function EventsScreen() {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const res = await eventsService.getAll({ nagarsevak_id: nagarsevakId, limit: 500 });
      setEvents(Array.isArray(res) ? res : res.events || []);
    } catch (e) { showToast('error', 'Failed to load events.'); }
    setLoading(false);
    setRefreshing(false);
  }, [nagarsevakId]);
  ...
  const handleSave = async () => {
    if (!form.title.trim() || !form.date) { showToast('error', 'Title and date are required.'); return; }
    setSaving(true);
    try {
  ...
  const handleDelete = (e) => {
    Alert.alert('Delete Event', `Delete "${e.title}"?`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try { await eventsService.delete(e.id); showToast('success', 'Deleted.'); load(true); }
  ...
  const TYPE_EMOJIS = { Meeting: '🤝', Rally: '📣', Cultural: '🎭', Sports: '🏆', Social: '🌟', 'Health Camp': '🏥', Other: '📅' };

  const renderItem = ({ item: e }) => {
    const isPast = new Date(e.date) < now;
    return (
      <TouchableOpacity style={[styles.card, isPast && styles.cardPast]} onPress={() => setViewItem(e)} activeOpacity={0.7}>
        <View style={styles.cardDateCol}>
          <Text style={styles.cardDateNum}>{new Date(e.date).getDate?.() || '—'}</Text>
          <Text style={styles.cardDateMon}>{new Date(e.date).toLocaleString?.('en', { month: 'short' }) || ''}</Text>
        </View>
        <View style={styles.cardContent}>
          <Text style={styles.cardTitle} numberOfLines={1}>{TYPE_EMOJIS[e.type] || '📅'} {e.title}</Text>
          {e.venue ? <Text style={styles.cardVenue}>📍 {e.venue}</Text> : null}
          {e.time ? <Text style={styles.cardTime}>🕐 {e.time}</Text> : null}
          <View style={styles.cardMeta}>
            <View style={styles.metaChip}><Text style={styles.metaChipText}>{e.type}</Text></View>
            {e.expectedAttendance ? <View style={styles.metaChip}><Text style={styles.metaChipText}>👥 {e.expectedAttendance}</Text></View> : null}
          </View>
        </View>
        {canManage && (
          <View style={styles.cardBtns}>
            <TouchableOpacity style={styles.editBtn} onPress={() => { setEditItem(e); setForm({ ...defForm, ...e }); setShowForm(true); }}>
    ...
    <View style={[styles.container, { paddingHorizontal: padding }]}>
      ...
      <View style={[styles.statsGrid, { paddingTop: padding }]}>
        {[
          ...
          { icon: '🔜', label: 'Upcoming', value: stats.upcoming, color: '#dbeafe' },
          { icon: '✅', label: 'Past', value: stats.past, color: '#d1fae5' },
          { icon: '🗓️', label: 'This Month', value: stats.thisMonth, color: '#fef3c7' },
        ].map((s, i) => <StatCard key={i} {...s} style={{ flex: 1 }} />)}
      </View>

      <View style={styles.filterRow}>
        <SearchBar value={search} onChangeText={setSearch} placeholder="Search events..." style={{ flex: 1 }} />
        {canManage && (
      ...
      <FilterChips options={['All', 'Upcoming', 'Past']} value={filter} onChange={setFilter} style={{ marginBottom: SPACING.md }} />
      <Text style={styles.count}>{filtered.length} events</Text>

      {loading ? <LoadingSpinner text="Loading events..." full /> : (
        <FlatList
          ...
          ListEmptyComponent={<EmptyState icon="📅" title="No events found" />}
        />
      )}

      {/* Detail */}
      <ModalSheet visible={!!viewItem} onClose={() => setViewItem(null)} title="Event Details">
        {viewItem && (
          <View>
            <Text style={styles.detailTitle}>{TYPE_EMOJIS[viewItem.type] || '📅'} {viewItem.title}</Text>
            {[
              ['Type', viewItem.type],
      ...
      {/* Form */}
      <ModalSheet visible={showForm} onClose={() => { setShowForm(false); setEditItem(null); setForm(defForm); }} title={editItem ? 'Edit Event' : 'New Event'}>
        <FormGroup label="Event Title" required>
          <Input value={form.title} onChangeText={(v) => setForm(f => ({ ...f, title: v }))} placeholder="Event name" />
        </FormGroup>
        <FormGroup label="Type">
          <SelectField value={form.type} options={EVENT_TYPES} onChange={(v) => setForm(f => ({ ...f, type: v }))} />
        </FormGroup>
        <FormGroup label="Date" required>
          <Input value={form.date} onChangeText={(v) => setForm(f => ({ ...f, date: v }))} placeholder="YYYY-MM-DD" />
        </FormGroup>
        <FormGroup label="Time">
          <Input value={form.time} onChangeText={(v) => setForm(f => ({ ...f, time: v }))} placeholder="e.g. 10:00 AM" />
        </FormGroup>
        <FormGroup label="Venue">
          <Input value={form.venue} onChangeText={(v) => setForm(f => ({ ...f, venue: v }))} placeholder="Venue / location" />
        </FormGroup>
        <FormGroup label="Expected Attendance">
          <Input value={form.expectedAttendance} onChangeText={(v) => setForm(f => ({ ...f, expectedAttendance: v }))} placeholder="e.g. 200" keyboardType="numeric" />
        </FormGroup>
        <FormGroup label="Organizer">
          <Input value={form.organizer} onChangeText={(v) => setForm(f => ({ ...f, organizer: v }))} placeholder="Organizer name" />
        </FormGroup>
        <FormGroup label="Description">
          <Input value={form.description} onChangeText={(v) => setForm(f => ({ ...f, description: v }))} placeholder="Event details..." multiline numberOfLines={3} style={{ minHeight: 80, textAlignVertical: 'top', paddingTop: SPACING.md }} />
        </FormGroup>
        <TouchableOpacity style={[styles.saveBtn, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving}>
          <Text style={styles.saveBtnText}>{saving ? 'Saving...' : editItem ? 'Update Event' : 'Create Event'}</Text>
        </TouchableOpacity>
      </ModalSheet>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  cardDateCol: { width: 44, height: 52, backgroundColor: COLORS.primary, borderRadius: RADIUS.lg, alignItems: 'center', justifyContent: 'center' },
  cardDateNum: { ...TYPOGRAPHY.h4, color: COLORS.white },
  cardDateMon: { ...TYPOGRAPHY.caption, color: 'rgba(255,255,255,0.8)', fontWeight: '600' },
  cardContent: { flex: 1 },
  ...
  detailTitle: { ...TYPOGRAPHY.h3, color: COLORS.gray800, marginBottom: SPACING.lg },
