...
export default function VolunteersScreen() {
  ...
  const load = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        const res = await volunteersService.getAll({
  ...
  const handleSubmitReport = async () => {
    if (!reportForm.task.trim() || !reportForm.date) {
      showToast('error', 'Date and task are required.');
      return;
    }
    setSaving(true);
    try {
  ...
  const handleDelete = v => {
    Alert.alert('Remove Volunteer', `Remove ${v.name}?`, [
      {text: 'Cancel', style: 'cancel'},
      {
        ...
        onPress: async () => {
          try {
            await volunteersService.delete(v.id);
      ...
      <View style={styles.cardMeta}>
        {v.ward ? (
          <View style={styles.metaChip}>
            <Text style={styles.metaChipText}>Ward {v.ward}</Text>
          </View>
        ) : null}
        {v.skills ? (
          <View style={styles.metaChip}>
  ...
  return (
    <>
      ...
      <View style={[styles.container, {paddingHorizontal: padding}]}>
        ...
        {loading ? (
          <LoadingSpinner text="Loading volunteers..." full />
        ) : (
          <FlatList
            data={filtered}
            keyExtractor={v => v.id}
            ...
            ListEmptyComponent={
              <EmptyState icon="🤝" title="No volunteers found" />
            }
          />
        )}
        ...
        <ModalSheet
          ...
          title={editItem ? 'Edit Volunteer' : 'Add Volunteer'}>
          ...
          <FormGroup label="Skills">
            <Input
              ...
              placeholder="e.g. Communication, Event Mgmt"
            />
          </FormGroup>
          ...
          <FormGroup label="Join Date">
            <Input
              value={form.joinDate}
              onChangeText={v => setForm(f => ({...f, joinDate: v}))}
        ...
        <ModalSheet
          ...
          title={`Report for ${selectedVol?.name}`}>
          <FormGroup label="Date" required>
            <Input
              value={reportForm.date}
              onChangeText={v => setReportForm(f => ({...f, date: v}))}
              placeholder="YYYY-MM-DD"
            />
          </FormGroup>
          <FormGroup label="Task Performed" required>
            <Input
              value={reportForm.task}
              onChangeText={v => setReportForm(f => ({...f, task: v}))}
          ...
          <FormGroup label="Hours Worked">
            <Input
              ...
              onChangeText={v => setReportForm(f => ({...f, hours: v}))}
              placeholder="e.g. 4"
              keyboardType="numeric"
            />
          </FormGroup>
          <FormGroup label="Status">
            <SelectField
              ...
              onChange={v => setReportForm(f => ({...f, status: v}))}
            />
          </FormGroup>
          <FormGroup label="Notes">
            <Input
              value={reportForm.note}
              onChangeText={v => setReportForm(f => ({...f, note: v}))}
