...
export default function ScheduleScreen() {
  ...
  const handleSave = async () => {
    if (!form.title.trim() || !form.date) {
      showToast('error', 'Title and date required.');
      return;
    }
    setSaving(true);
    try {
      ...
      <View style={styles.cardContent}>
        ...
        <View style={styles.cardMeta}>
          <View style={styles.chip}>
            <Text style={styles.chipText}>{s.type}</Text>
          </View>
          {s.attended ? (
          ...
          {new Date(s.date) < now && (
            <TouchableOpacity
              style={[
                styles.editBtn,
                {
                  backgroundColor: s.attended
                    ? COLORS.gray100
                    : COLORS.successLight,
                },
              ]}
              onPress={() => handleAttendance(s, !s.attended)}>
              <Icon
                name={s.attended ? 'close' : 'checkmark'}
                size={13}
                color={s.attended ? COLORS.gray500 : COLORS.success}
              />
            </TouchableOpacity>
          )}
        </View>
  ...
  return (
    <ScrollView>
      <View style={[styles.container, {paddingHorizontal: padding}]}>
        ...
        />
        {loading ? (
          <LoadingSpinner text="Loading schedule..." full />
        ) : (
          <FlatList
            ...
            ListEmptyComponent={
              <EmptyState icon="📋" title="No schedule items" />
            }
          />
        )}
        <ModalSheet
          ...
          <TouchableOpacity
            ...
            <Text style={styles.saveBtnText}>
              {saving ? 'Saving...' : editItem ? 'Update' : 'Add to Schedule'}
            </Text>
          </TouchableOpacity>
        </ModalSheet>
      </View>
